...
@pytest.mark.parametrize("function, arity, args, expected_exception", [
    (lambda x: f'<{x}>', 0, (1,), TypeError),
    (lambda x: f'<{x}>', 1, (1, 3), TypeError),
    (lambda x: f'<{x}>', -1, (1,), ValueError),
])
...
@pytest.mark.parametrize("function, arity, args, expected_output", [
    (lambda: f'<>', 0, (), '<>'),
    (lambda x: f'<{x}>', 1, (1,), '<1>'),
    (print, 2, (1, 2), None),  
])
...
def test_uncurry(function, arity, args, expected_output):
